...
contract GeniusExecutorTest is Test {
    ...
    address payable public routerAddress =
        payable(0xb4315e873dBcf96Ffd0acd8EA43f689D8c20fB30);
    ...
    function testAggregateWithoutPermit2() public {
        ...
        assertEq(
            USDC.balanceOf(holderOne),
            100 ether,
            "Holder One should still have 100 USDC"
        );
        assertEq(
            USDC.balanceOf(holderTwo),
            100 ether,
            "Holder Two should still have 100 USDC"
        );

        // Verify that the mock swaps occurred
        address mockWETHAddress = mockRouter.mockTokens(address(WETH));
        assertEq(
            mockWETHAddress != address(0),
            true,
            "Mock WETH token should have been created"
        );
        assertEq(
            MockERC20(mockWETHAddress).balanceOf(address(EXECUTOR)),
            mockRouter.MINT_AMOUNT() * 2,
            "Executor should have received mock WETH"
        );
        ...
        assertEq(
            ...
            0,
            "MockDEXRouter should not have received any USDC"
        );

        // Verify that the correct amount of mock WETH was minted
        assertEq(
            MockERC20(mockWETHAddress).totalSupply(),
            mockRouter.MINT_AMOUNT() * 2,
            "Total supply of mock WETH should be MINT_AMOUNT * 2"
        );
    }
    ...
    function testAggregateWithPermit2() public {
        ...
        assertEq(
            wavaxContract.balanceOf(receiverOne),
            1 ether,
            "Receiver One should have 1 WAVAX"
        );
        assertEq(
            meowContract.balanceOf(receiverTwo),
            2 ether,
            "Receiver Two should have 2 MEOW"
        );
        assertEq(
            wavaxContract.balanceOf(address(EXECUTOR)),
            0,
            "Executor should have no WAVAX"
        );
        assertEq(
            meowContract.balanceOf(address(EXECUTOR)),
            0,
            "Executor should have no MEOW"
        );
        assertEq(
            wavaxContract.balanceOf(TRADER),
            99 ether,
            "Trader should have 99 WAVAX"
        );
        assertEq(
            meowContract.balanceOf(TRADER),
            98 ether,
            "Trader should have 98 MEOW"
        );
    }
    ...
    function testTokenSwapAndDeposit() public {
        ...
        assertEq(
            ...
            5 ether,
            "Executor should have 4 test tokens"
        );
        assertEq(
            VAULT.stablecoinBalance(),
            5 ether,
            "Vault should have 4 test tokens available"
        );
        assertEq(
            VAULT.availableAssets(),
            0 ether,
            "Vault should have 0% of test tokens available"
        );
        assertEq(
            VAULT.reservedAssets(),
            5 ether,
            "Vault should have 100% of tokens reserved"
        );
        assertEq(
            VAULT.totalStakedAssets(),
            0,
            "Vault should have 0 test tokens staked"
        );
    }
    ...
    function testNativeSwapAndDeposit() public {
        ...
        assertEq(
            USDC.balanceOf(address(VAULT)),
            50 ether,
            "Executor should have 10 test tokens"
        );
        assertEq(
            VAULT.stablecoinBalance(),
            50 ether,
            "Vault should have 10 test tokens available"
        );
        assertEq(
            VAULT.availableAssets(),
            0 ether,
            "Vault should have 0 test tokens available"
        );
        assertEq(
            VAULT.reservedAssets(),
            50 ether,
    ...
    function testMultiSwapAndDeposit() public {
        ...
        assertEq(
            USDC.balanceOf(address(VAULT)),
            120 ether,
            "Executor should have 120 test tokens"
        );
        assertEq(
            USDC.balanceOf(holderOne),
            90 ether,
            "Holder One should have 90 test tokens"
        );
        assertEq(
            USDC.balanceOf(holderTwo),
            90 ether,
            "Holder Two should have 90 test tokens"
        );
        assertEq(
            USDC.balanceOf(TRADER),
            traderBalance - 100 ether,
            "Trader should have expected balance"
        );
        assertEq(
            VAULT.stablecoinBalance(),
            120 ether,
            "Vault should have 120 test tokens available"
        );
        assertEq(
            VAULT.availableAssets(),
            0 ether,
        ...
        assertEq(
            VAULT.reservedAssets(),
            120 ether,
            "Vault should have 0 test tokens reserved"
        );
    }
    ...
    function testDepositToVault() public {
        ...
        assertEq(
            VAULT.balanceOf(TRADER),
            depositAmount,
            "Trader should have received vault shares"
        );
        assertEq(
            USDC.balanceOf(TRADER),
            90 ether,
            "Trader should have 90 USDC left"
        );
    }
    ...
    function testWithdrawFromVault() public {
        ...
        assertEq(
            VAULT.balanceOf(TRADER),
            9 ether,
            "Trader should have remaining vault shares"
        );
        assertEq(
            USDC.balanceOf(TRADER),
            91 ether,
